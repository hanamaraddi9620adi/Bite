...
@Component({
  ...
  standalone: false,
  template: `
    <div class="api-error" *ngIf="errorMessage">{{ errorMessage }}</div>
...
export class AuthComponent {
  ...
  form!: FormGroup;
  errorMessage = '';
  constructor(private readonly formBuilder: FormBuilder, private readonly route: ActivatedRoute, private readonly router: Router, private readonly authService: AuthService) { this.isSignup = this.route.snapshot.data['mode'] === 'signup'; this.form = this.formBuilder.group({ name: [''], email: ['', [Validators.required, Validators.email]], password: ['', [Validators.required, Validators.minLength(6)]], terms: [false] }); if (this.isSignup) this.form.controls['name'].addValidators(Validators.required); }
  fieldInvalid(field: string) { const control = this.form.get(field); return !!control && control.invalid && (control.dirty || control.touched); }
  submit() { this.form.markAllAsTouched(); this.errorMessage = ''; if (!this.form.valid || (this.isSignup && !this.form.controls['terms'].value)) return; const request = this.isSignup ? this.authService.signup(this.form.controls['name'].value, this.form.controls['email'].value, this.form.controls['password'].value) : this.authService.login(this.form.controls['email'].value, this.form.controls['password'].value); request.subscribe({ next: () => this.router.navigate(['/']), error: (error: HttpErrorResponse) => { this.errorMessage = error.status === 409 ? 'An account with this email already exists.' : 'We could not sign you in. Check your email and password.'; } }); }
}
